"""
Integration tests for Chat, Auth, Contact, and Streaming endpoints.
"""
...
async def test_post_chat_endpoint(async_client: AsyncClient):
    ...
    response = await async_client.post(
        "/api/v1/chat",
        json={"message": "Can I download your resume?", "session_id": "test_session_100"},
    )
    assert response.status_code == 200
    data = response.json()
...
async def test_sse_stream_endpoint(async_client: AsyncClient):
    ...
    response = await async_client.get("/api/v1/chat/stream?message=Hello+Alemu")
    assert response.status_code == 200
